...
const MagDurChart = (props: { Width: number, Height: number }) => {
    ...
    function GetMagDurPoints(): Promise<iPoint[]> {
        return new Promise((res,rej) => $.ajax({
            type: "GET",
    ...
    function Initialize(curves: iCurve[], points: iPoint[]) {
        const margin = { top: 15, right: 20, bottom: 60, left: 40 };
        const svgWidth = props.Width - margin.left - margin.right;
        const svgHeight = props.Height - margin.top - margin.bottom;
        ...
        let zoom = d3Zoom().on('zoom', function (event: D3ZoomEvent<SVGElement, iCurve>) {
    ...
    return (
        <div ref={chart} style={{ height: props.Height, width: props.Width }}>
            ...
            <button style={{ position: 'absolute', top: 10, left: svgWidth }} onClick={resetZoom}>Reset</button>
        </div>
    )
}
