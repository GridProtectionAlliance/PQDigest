...
foreach ($repo in $gemstoneRepos) {
    ...
    if ($changes -gt 0) {
        ...
        $prLink = GeneratePR -Repository "Gemstone/$repo" -Title "Release Update" -Body "This PR was Generated by a release of PQDigest" -mainBranch "master" -organization "Gemstone"
        $prs += $prLink
    }
}

# Add Comments to the PR with the Open PRs in the PQDigest Repo
if ($prs.Count -gt 0) {
...
& "$PSScriptRoot\GithubComment.ps1" -Comment $commentBody -BranchName "$DevelopmentBranchName" -GithubToken $GithubToken -RepoOwner "GridProtectionAlliance" -RepoName "PQDigest"
